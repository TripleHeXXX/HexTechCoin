...
const char * const BITCOIN_CONF_FILENAME = "hextechcoin.conf";
const char * const BITCOIN_PID_FILENAME = "hextechcoind.pid";
...
#else
    const char* pszModule = "hextechcoin";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "HexTechCoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/HexTechCoin";
#else
    // Unix
    return pathRet / ".hextechcoin";
#endif
#endif
}
